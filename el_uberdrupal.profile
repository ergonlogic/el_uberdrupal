<?php
// $Id: el_uberdrupal.profile,v 1.3.2.4 2010/08/20 15:51:29 rszrama Exp $
 ...
 */
function el_uberdrupal_profile_details() {
  return array(
    'name' => 'UberDrupal',
 ...
 */
function el_uberdrupal_profile_modules() {
  return array(
    // Core Drupal modules:
    'block',
    'filter',
 ...
 */
function el_uberdrupal_profile_task_list() {
  return array(
 ...
 */
function el_uberdrupal_profile_tasks(&$task, $url) {
  switch ($task) {
    ...
    case 'profile':
      ...
      el_uberdrupal_install_module('admin_menu');

      // Install Acquia Prosper if the necessary module and themes exist.
      _el_uberdrupal_install_theme();
      ...
      _el_uberdrupal_create_nodes();

      // Setup some default menu items.
      _el_uberdrupal_setup_menu_items();
 ...
 */
function el_uberdrupal_store_settings_form(&$form_state, $url) {
  $form = array(
    '#action' => $url,
    '#redirect' => FALSE,
  );
...
function el_uberdrupal_store_settings_form_submit() {
 ...
 *   URL of current installer page, provided by installer.
 */
function el_uberdrupal_features_form($form_state, $url) {
  $form = array(
    '#action' => $url,
    '#redirect' => FALSE,
  );
  ...
  foreach (el_uberdrupal_list_features() as $filename => $file) {
...
function el_uberdrupal_features_form_submit() {
  $postprocess = array();
 ...
 */
function el_uberdrupal_list_features() {
  return file_scan_directory('profiles/el_uberdrupal', '.*\.pkg.inc');
}
 ...
 */
function el_uberdrupal_install_module($module) {
  // If an array was passed...
  if (is_array($module)) {
    // Loop through the array and install each one.
    foreach ($module as $mod) {
      el_uberdrupal_install_module($mod);
    }
  }
  else {
 ...
 * Setup the page content type.
 */
function _el_uberdrupal_setup_page() {
...
function _el_uberdrupal_install_theme() {
  ...
  if (drupal_get_path('module', 'skinr') && in_array('fusion_core', $themes) && in_array('acquia_prosper', $themes)) {
    // First install Skinr.
    el_uberdrupal_install_module('skinr');
  ...
  _el_uberdrupal_setup_blocks();
}

// Configure the blocks for Acquia Prosper.
function _el_uberdrupal_setup_blocks() {
  $blocks = array(
...
function _el_uberdrupal_setup_menu_items() {
  $items = array(
...
function _el_uberdrupal_create_nodes() {
